  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  RCLCPP_INFO(get_node()->get_logger(), "RIGHT Finger Mimic: main='%s', finger='%s', mult=%.3f, offset=%.4f",
  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  RCLCPP_INFO(get_node()->get_logger(), "RIGHT Finger activated: main=%.5f -> finger=%.5f", main_pos, finger_pos);
  return controller_interface::CallbackReturn::SUCCESS;
}
