#include "robotiq_controllers/left_finger_mimic_controller.hpp"
#include <algorithm>
...
LeftFingerMimicController::LeftFingerMimicController()
: controller_interface::ControllerInterface()
{
}

controller_interface::CallbackReturn LeftFingerMimicController::on_init()
{
  try {
    auto_declare<std::string>("main_joint", "hande_finger_distance");
    auto_declare<std::string>("finger_joint", "hande_left_finger_joint");
...
controller_interface::CallbackReturn LeftFingerMimicController::on_configure(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  RCLCPP_DEBUG(get_node()->get_logger(), "LEFT Finger Mimic: main='%s', finger='%s', mult=%.3f, offset=%.4f",
...
LeftFingerMimicController::command_interface_configuration() const
{
...
controller_interface::InterfaceConfiguration 
LeftFingerMimicController::state_interface_configuration() const
{
  controller_interface::InterfaceConfiguration config;
  config.type = controller_interface::interface_configuration_type::INDIVIDUAL;
...
controller_interface::CallbackReturn LeftFingerMimicController::on_activate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  RCLCPP_DEBUG(get_node()->get_logger(), "LEFT Finger activated: main=%.5f -> finger=%.5f", main_pos, finger_pos);
  return controller_interface::CallbackReturn::SUCCESS;
}

controller_interface::CallbackReturn LeftFingerMimicController::on_deactivate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  return controller_interface::CallbackReturn::SUCCESS;
}

controller_interface::return_type LeftFingerMimicController::update(
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  RCLCPP_DEBUG_THROTTLE(get_node()->get_logger(), *get_node()->get_clock(), 1000,
    "LEFT: main=%.5f -> finger=%.5f", main_pos, finger_pos);
...
PLUGINLIB_EXPORT_CLASS(robotiq_controllers::LeftFingerMimicController, controller_interface::ControllerInterface)